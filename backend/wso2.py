...
class AccessTokenManager(object):
    ...
    def access_token(self) -> str:
        ...
        if self._token is None or self.expire <= _now:
            ...
            self.expire = _now + timedelta(seconds=info['expires_in'])
            self._token = info['access_token']
        else:
            expiration = int((self.expire-_now).total_seconds())
            logger.debug(f"Token will expire in {expiration:d} seconds.")
