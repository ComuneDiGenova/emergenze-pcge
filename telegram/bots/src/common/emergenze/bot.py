# Reference: https://docs.aiogram.dev/en/v2.25.1/
...
def esegui_query(connection, query, query_type):
    ...
    if isinstance(query_type, str) is False:
        logging.warning(
            "query type must be a str. The query {} was not executed"
            .format(query)
        )
        return 1
    elif query_type != "i" and query_type != "u" and query_type != "s":
        logging.warning(
            "query type non recgnized for query: {}. The query was not executed"
            .format(
                query
            )
        )
    ...
    try:
        curr.execute("SET TIME ZONE 'Europe/Rome';")
    except Exception as e:
        logging.warning("Impossibile impostare timezone: {}".format(e))
...
def keyboard(kb_config):
    ...
    for rows in kb_config:
        btn = types.InlineKeyboardButton(callback_data=rows[0], text=rows[1])
...
async def callback(callback_query: types.CallbackQuery):
    ...
    if (
        ...
        or callback_query.data == "4"
        or callback_query.data == "6"
        or callback_query.data == "8"
    ):
        await bot.delete_message(
            callback_query.from_user.id, callback_query.message.message_id
        )
        query_time_operativo = """INSERT INTO users.t_presenze (operativo, data_inizio, durata, id_telegram, data_fine_hp)
        VALUES(true, now(), {0}, '{1}', now() + interval '{0} hours')""".format(
        ...
        if result == 0:
            ...
            testo = "{} Caro {}, la tua presenza è stata registrata. Il tuo turno comincia adesso e terminerà alle ore {}".format(
                emoji.emojize(":white_check_mark:", use_aliases=True),
                callback_query.from_user.first_name,
                termine_turno.strftime("%H:%M"),
            )
            await bot.send_message(callback_query.from_user.id, text=testo)
        else:
            await bot.send_message(
                callback_query.from_user.id,
                """{} Si è verificato un problema, e la registrazione non è anadata a buon fine:
                               \nSe visualizzi questo messaggio prova a contattare un tecnico""".format(
        ...
        query_fine_turno = "UPDATE users.t_presenze SET operativo=false, data_fine=now() WHERE operativo =true and id_telegram ='{}'".format(
            callback_query.from_user.id
        )
        ...
        if result == 0:
            ...
            await bot.send_message(
                callback_query.from_user.id,
                "Caro {}, il tuo turno è stato chiuso correttamente.".format(
                    callback_query.from_user.first_name
                ),
            )
        else:
            ...
            await bot.send_message(
                callback_query.from_user.id,
                "Caro {}, si è verificato un problema nella chiusura del tuo turno.\nPer favore contatta un amministratore di sistema".format(
                    callback_query.from_user.first_name
                ),
            )
        ...
        await bot.send_message(
            callback_query.from_user.id,
            "Hai annullato il comando e il tuo turno proseguirà. Utilizza nuovamente il comando /registra_uscita per terminare il turno",
        )
...
async def send_help(message: types.Message):
    ...
    await bot.send_message(
        message.chat.id,
        """Le funzioni BOT del Sistema di Gestione Emergenze del Comune di Genova sono:\n
    ...
    \n\nTramite questo BOT potrai anche ricevere notifiche dal sistema. Per fare questo devi inserire il tuo telegram_id nel portale e attivare le notifiche.""".format(
...
async def send_welcome(message: types.Message):
    ...
    """
    await message.reply(
        "Ciao {}, il tuo codice (telegram id) da inserire nell'applicazione è {}".format(
...
async def start_cmd_handler(message: types.Message):
    ...
    keyboard_markup.row(
        ...
        types.InlineKeyboardButton(
            ...
            url="https://emergenze.comune.genova.it/emergenze",
        ),
    )

    await message.reply(
        "Ciao {}, cliccando sul seguente bottone verrai reindirizzato al sito del Sistema di Getione Emergenze".format(
            message.from_user.first_name
        ),
...
async def send_welcome(message: types.Message):
    ...
    query_tlgrm_id = (
        "select * from users.utenti_sistema where telegram_id ='{}'".format(
            message.chat.id
        )
    )
    ...
    if registrato == 1:
        ...
        query_operativo = "select * from users.t_presenze where id_telegram ='{}' and operativo = 't'".format(
            message.chat.id
        )
        ...
        if operativo == 1:
            ...
            await bot.send_message(
                message.chat.id,
                """{} Caro {}, in questo momento risulti già operativo. 
                                   \nProva a chiudere il tuo turno con il comando /registra_uscita, oppure contatta un amministratore di sistema""".format(
                    emoji.emojize(":warning:", use_aliases=True),
                    message.from_user.first_name,
                ),
            )
            ...
            await bot.send_message(
                ...
                \n\n{} Quante ore prevedi di rimanere operativo?""".format(
                    emoji.emojize(":white_check_mark:", use_aliases=True),
                    message.from_user.first_name,
        ...
        await bot.send_message(
            message.chat.id,
            """{} Il tuo utente non è registrato nel sistema e pertanto non puoi usare questo comando.
                               \nContatta un amministratore di sistema per registrarti, e dopo esser stato abilitato ripeti questo comando""".format(
...
async def send_welcome(message: types.Message):
    ...
    controllo_operativo = "SELECT operativo, data_inizio, durata, data_fine, id, id_telegram FROM users.t_presenze where operativo =true and id_telegram ='{}'".format(
        message.chat.id
    )
    ...
    if result_operativo == 1:
        ...
        await bot.send_message(
            message.chat.id,
            """{} Caro {}, in questo momento non risulti operativo. 
                                   \nProva ad iniziare il tuo turno con il comando /registra_presenza, oppure contatta un amministratore di sistema""".format(
                emoji.emojize(":warning:", use_aliases=True),
                message.from_user.first_name,
            ),
        )
        ...
        await bot.send_message(
            message.chat.id,
            text="Sei sicuro di voler terminare ora il tuo turno?",
            reply_markup=keyboard(
                [
...
async def process_presa(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if message.text not in data["rivi"].keys():
            await message.reply(
                "Il rio inserito non è valido. Seleziona il rio usando le opzioni presenti sulla tastiera."
            )
        else:
            ...
            await bot.send_message(
                message.chat.id,
                "Hai inserito {}".format(data["rivo"]),
                reply_markup=markup_old,
            )
            await FormPresa.next()
            markup_new = types.ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
            markup_new.add(
                "Verde {}".format(emoji.emojize(":green_circle:", use_aliases=True)),
                "Giallo {}".format(emoji.emojize(":yellow_circle:", use_aliases=True)),
                "Rosso {}".format(emoji.emojize(":red_circle:", use_aliases=True)),
            )
            ...
            await message.reply(
                "Come valuti la mira per il rivo scelto?", reply_markup=markup_new
            )
...
@dp.message_handler(
    ...
    state=FormPresa.mira,
)
async def process_gender_invalid(message: types.Message):
    return await message.reply(
        "Il valore inserito non è valido. Seleziona un valore usando le opzioni presenti sulla tastiera."
    )
...
async def process_presa(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        """.format(
            data["rivi"][data["rivo"]][0], id_lettura
        )

        query_log = """INSERT INTO varie.t_log (schema,operatore, operazione) VALUES ('geodb','{}', 'Inserita lettura mira . {}')""".format(
            data["user"], data["rivi"][data["rivo"]][0]
        )
        ...
        if inserimento_mira == 0 and resultlog == 0:
            ...
            await bot.send_message(
                message.chat.id,
                """Si è verificato un problema nell'inserimento della mira nel database.
                                   \nSe visualizzi questo messaggio prova ad usare nuovamente il comando /inserisci_mira o a contattare un tecnico""",
                reply_markup=markup,
            )
...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    query_telegram_id = (
        "select * from users.v_utenti_sistema where telegram_id ='{}'".format(
            message.chat.id
        )
    )
    ...
    if registered_user == 1:
        await bot.send_message(
            message.chat.id,
            """{} Si è verificato un problema, e non è possibile capire se il tuo utente è registrato nel sistema:
                            \nSe visualizzi questo messaggio prova a contattare un tecnico""".format(
                emoji.emojize(":warning:", use_aliases=True)
            ),
        )
        ...
        where vcs.matricola_cf ='{}' and vcs.data_end is null and vsmlu.id_stato_sopralluogo =2""".format(
            registered_user[0][0]
        )
        ...
        if pm_assegnato == 1:
            ...
            queryfoc = "SELECT * FROM eventi.v_foc WHERE id_evento={} and data_ora_fine_foc > now()".format(
                id_evento
            )
            ...
            if foc == 1:
                await bot.send_message(
                    message.chat.id,
                    """{} Si è verificato un problema sulla verifica della presenza di una F.O.C.:
                            \nSe visualizzi questo messaggio prova a contattare un tecnico""".format(
                        emoji.emojize(":warning:", use_aliases=True)
                    ),
                )
                ...
                query_mire = """SELECT p.id, concat(p.nome,' (', replace(p.note,'LOCALITA',''),')') as nome
                       ...
                       order by nome""".format(
                    nome_perc, percorso_assegnato
                )
                ...
                if len(testo_dati) == 0:
                    await bot.send_message(
                        message.chat.id,
                        """Non ci sono rivi per cui inserire la mira.
                                           \nDurante la F.O.C. {}, infatti il percorso {}, che è stato assegnato alla tua squadra, è disabilitato.""".format(
                            foc[0][2], percorso_assegnato
                        ),
                    )
                    ...
                    await bot.send_message(
                        message.chat.id,
                        "Per quale tra i seguenti rivi vuoi inserire la mira?",
                        reply_markup=markup,
                    )
                ...
                await bot.send_message(
                    message.chat.id,
                    """{} Al momento non risultano F.O.C. attive sul tuo evento, per cui non puoi usare questo comando.""".format(
                        emoji.emojize(":warning:", use_aliases=True)
                    ),
                )
            ...
            await bot.send_message(
                message.chat.id,
                """{} Al momento non risultano presidi mobili assegnati e/o accettati alla tua squadra, per cui non puoi usare questo comando.""".format(
                    emoji.emojize(":warning:", use_aliases=True)
                ),
            )
...
async def process_testo(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await message.reply(
            "{} Vuoi allegare una foto alla comunicazione inserita o inviarla senza foto?".format(
                emoji.emojize(":camera:", use_aliases=True)
            ),
            reply_markup=markupnext,
        )


@dp.message_handler(
    lambda message: message.text not in ["Foto", "Invia"],
    state=FormComunicazione.foto_flag,
)
async def process_gender_invalid(message: types.Message):
    return await message.reply(
        "Comando non valido. Seleziona il comando usando le opzioni presenti sulla tastiera."
    )
...
async def process_presa(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if data["foto_com"] == "Foto":
            ...
            await message.reply(
                "Scatta una foto dal tuo dispositivo ", reply_markup=markupend
            )
            await FormComunicazione.next()
        else:
            ...
            if data["tipo"] == "presidio mobile":
                ...
                qinsertpm = """INSERT INTO segnalazioni.t_comunicazioni_sopralluoghi_mobili(id_sopralluogo, testo) VALUES ({},'{}')""".format(
                    data["id_pm"], data["testo_com"]
                )
                query_logpm = """INSERT INTO varie.t_log (schema,operatore, operazione) VALUES ('segnalazioni','{}', 'Inviata comunicazione a PC (presidio mobile {})')""".format(
                    data["user"], data["testo_com"]
                )
                con = psycopg2.connect(
                    host=conn.ip,
                    dbname=conn.db,
                ...
                if resultinspm == 1 or resultlogpm == 1:
                    await message.reply(
                        "Si è verificato un problema tecnico nell'invio della comunicazione",
                        reply_markup=markupend,
                    )
                else:
                    await message.reply(
                        "{}{} Comunicazione senza foto inviata".format(
                ...
                qinsertcom = """INSERT INTO segnalazioni.t_comunicazioni_segnalazioni(id_lavorazione, mittente, testo) VALUES({},'{}','{}') """.format(
                    data["id_lavorazione"], data["mittente"], data["testo_com"]
                )
                query_log = """ INSERT INTO varie.t_log (schema,operatore, operazione) VALUES ('segnalazioni','{}', 'Inviata comunicazione a PC su segnalazione {}') """.format(
                    data["user"], data["id_segnalazione"]
                )
...
async def process_foto(message: types.Message, state: FSMContext):
    if message.content_type == types.ContentType.PHOTO:
        async with state.proxy() as data:
            ...
            photo_name = "{}_{}.jpg".format(
                datetime.now().strftime("%Y%m%d%H%M"), message.chat.id
            )

            rel_destination = (
                f"emergenze_uploads/telegram/e_{data['id_evento']}/pm_{data['id_pm']}"
            )
            if data["tipo"] == "presidio mobile":
            ...
            if resultins == 1 or resultlog == 1:
                ...
                await message.reply(
                    f"{emoji.emojize(':arrow_right:', use_aliases=True)} {emoji.emojize(':email:', use_aliases=True)} Comunicazione con foto inviata",
                    reply_markup=markupend,
                )
...
async def comunication(message: types.Message, state=FSMContext):
    ...
    if registered_user == 1:
        ...
        q1 = """select * from users.v_componenti_squadre vcs left join users.v_squadre_notifica vsn on vcs.id_squadra ::text = vsn.id ::text 
                where vcs.matricola_cf ='{}' and  (vsn.id_incarico_interno is not null or vsn.id_sopralluogo is not null or vsn.id_sm is not null) and vcs.data_end is null""".format(
            registered_user[0][0]
        )
        ...
        if incarico == 1:
            await bot.send_message(
                message.chat.id,
                """{} Si è verificato un problema, e non è possibile se alla tua squadra sono stati assegnati dei compiti:
                            \nSe visualizzi questo messaggio prova a contattare un tecnico""".format(
                    emoji.emojize(":warning:", use_aliases=True)
                ),
            )
            ...
            if incarico[0][-3]:
                ...
                queryii = """select * from users.v_componenti_squadre vcs left join segnalazioni.v_incarichi_interni vii on vcs.id ::text =vii.id_squadra ::text 
                            where vcs.matricola_cf ='{}' and vii.id_stato_incarico=2 and time_stop is null and vcs.data_end is null""".format(
                    registered_user[0][0]
                )
                ...
                if resultii == 1:
                    await bot.send_message(
                        message.chat.id,
                        """{} Si è verificato un problema, e non è possibile risalite all'id dell'evento:
                            \nSe visualizzi questo messaggio prova a contattare un tecnico""".format(
                            emoji.emojize(":warning:", use_aliases=True)
                        ),
                    )
                    ...
                    user = resultii[0][2]
                    mittente = "{} {} ({})".format(
                        resultii[0][4], resultii[0][5], resultii[0][6]
                    )
                    ...
                    await bot.send_message(
                        message.chat.id,
                        """{} Attenzione: l'incarico assegnato alla tua squadra potrebbe non esser ancora stato preso in carico, pertanto non è possibile inserire una comunicazione""".format(
                            emoji.emojize(":warning:", use_aliases=True)
                        ),
                    )
                ...
                querypf = """select * from users.v_componenti_squadre vcs left join segnalazioni.v_sopralluoghi vs on vcs.id_squadra  ::text = vs.id_squadra ::text
                           where vcs.matricola_cf ='{}' and vs.id_stato_sopralluogo =2 and vs.time_stop is null and vcs.data_end is null""".format(
                    registered_user[0][0]
                )
                ...
                if resultpf == 1:
                    ...
                    user = resultpf[0][2]
                    mittente = "{} {} ({})".format(
                        resultpf[0][4], resultpf[0][5], resultpf[0][6]
                    )
                    ...
                    await bot.send_message(
                        message.chat.id,
                        """{} Attenzione: il presidio fisso assegnato alla tua squadra potrebbe non esser ancora stato preso in carico, pertanto non è possibile inserire una comunicazione""".format(
                            emoji.emojize(":warning:", use_aliases=True)
                        ),
                    )
                ...
                querypm = """select * from users.v_componenti_squadre vcs left join segnalazioni.v_sopralluoghi_mobili vsm on vcs.id ::text =vsm.id_squadra ::text 
                            where vcs.matricola_cf ='{}' and vsm.id_stato_sopralluogo =2 and vsm.time_stop is null and vcs.data_end is null""".format(
                    registered_user[0][0]
                )
                ...
                if resultpm == 1:
                    ...
                    user = resultpm[0][2]
                    mittente = "{} {} ({})".format(
                        resultpm[0][4], resultpm[0][5], resultpm[0][6]
                    )
                    id_segnalazione = ""
                    id_lavorazione = ""
                    ...
                    if foc == 1:
                        ...
                        await bot.send_message(
                            message.chat.id,
                            """{} Stai cercando di inserire una comunicazione sul presidio mobile a te assegnato, ma al momento non risultano F.O.C. attive su questo tuo evento. Per questo motivo non puoi usare questo comando, riprova ad utilizzarlo quando ci sarà una F.O.C. attiva.""".format(
                                emoji.emojize(":warning:", use_aliases=True)
                            ),
                        )
                    ...
                    await bot.send_message(
                        message.chat.id,
                        """{} Attenzione: il presidio mobile assegnato alla tua squadra potrebbe non esser ancora stato preso in carico, pertanto non è possibile inserire una comunicazione""".format(
                            emoji.emojize(":warning:", use_aliases=True)
                        ),
                    )
            ...
            if inizio_com == True:
                ...
                await bot.send_message(
                    message.chat.id,
                    """Alla tua squadra risulta assegnato un {} sulla segnalazine {} nell\'ambito dell\'evento {}.
                                    \n{} Inserisci il testo della comunicazione che vuoi mandare alla centrale""".format(
                        tipo,
                        id_segnalazione,
                        id_evento,
            ...
            await bot.send_message(
                message.chat.id,
                """{} Alla tua squadra in questo momento non sono stati assegnati incarichi o presidi, pertanto non è possibile inserire una comunicazione.""".format(
                    emoji.emojize(":warning:", use_aliases=True)
                ),
            )
...
async def process_orario_invalid(message: types.Message):
    return await message.reply(
        "I minuti inseriti non sono validi, devi inserire un numero"
    )
...
async def process_orario(message: types.Message, state: FSMContext):
    ...
    # await message.reply("Hai indicato {} minuti quindi l'ora di inizio è {} circa.\n La presa in carico è:".format(data['orario'], timepreview), reply_markup=markup)
    await message.reply(
        "La presa in carico è regolare o parziale?", reply_markup=markup
    )
    # await state.finish ()
...
@dp.message_handler(
    lambda message: message.text not in ["Regolare", "Parziale"], state=Form.tipopresa
)
async def process_gender_invalid(message: types.Message):
    return await message.reply(
        "Il valore inserito non è valido. Seleziona il valore dalla tastiera."
    )
...
async def process_presa(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        await bot.send_message(
            ...
            md.text(
                ...
                md.text(
                    "Prevedi di iniziare l'incarico tra ",
                    md.bold(data["orario"]),
                    " minuti.\n",
                ),
                md.text(
                    "Una volta completato l'incarico ricordati di chiuderlo digitando /chiudo.\n"
                ),
            ),
            reply_markup=markup,
    ...
    query_incarico2 = """select us.matricola_cf, vc.id, vc.nome_squadra, viilu.id, vii.id_lavorazione, vii.id_profilo, vii.id_segnalazione
            ...
            where us.telegram_id = '{}' and vc.data_end is null and viilu.id_stato_incarico =1""".format(
        message.chat.id
    )
    ...
    if data["tipopresa"] == "Parziale":
        query_stato_p = "INSERT INTO segnalazioni.stato_incarichi_interni(id_incarico, id_stato_incarico, parziale) VALUES ({}, 2 , 'true');".format(
            incarico_assegnato2[0][3]
        )
        ...
        query_storico_p = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento)
            VALUES ({0}, ' Incarico interno {1} preso in carico (parzialmente) dalla seguente squadra: {2} - <a class="btn btn-info" href="dettagli_incarico_interno.php?id={1}"> Visualizza dettagli </a>');""".format(
            incarico_assegnato2[0][4],
            incarico_assegnato2[0][3],
            incarico_assegnato2[0][2],
        )
        storico_p = esegui_query(con, query_storico_p, "i")
    else:
        query_stato_r = "INSERT INTO segnalazioni.stato_incarichi_interni(id_incarico, id_stato_incarico) VALUES ({}, 2 );".format(
            incarico_assegnato2[0][3]
        )
        ...
        query_storico_r = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento)
            VALUES ({0}, ' Incarico interno {1} preso in carico dalla seguente squadra: {2} - <a class="btn btn-info" href="dettagli_incarico_interno.php?id={1}"> Visualizza dettagli </a>');""".format(
            incarico_assegnato2[0][4],
            incarico_assegnato2[0][3],
            incarico_assegnato2[0][2],
        )
        storico_r = esegui_query(con, query_storico_r, "i")
    query_log = "INSERT INTO varie.t_log (schema, operatore, operazione) VALUES ('segnalazioni','{}', 'Incarico interno {} preso in carico');".format(
        incarico_assegnato2[0][0], incarico_assegnato2[0][3]
    )
...
async def send_accetto(message: types.Message):
    ...
    if registered_user == 1:
        ...
        where vcs.matricola_cf = '{}' and vcs.data_end is null and viilu.id_stato_incarico =1""".format(
            registered_user[0][0]
        )
        ...
        if incarico_assegnato == 1:
            ...
            await message.reply(
                "Ciao {} hai accettato l'incarico {}. Tra quanti minuti sarai sul posto?".format(
                    message.from_user.first_name,
                    emoji.emojize(":thumbs_up:", use_aliases=True),
                )
            )
            ...
            await bot.send_message(
                message.chat.id,
                """{} Al momento non risultano incarichi assegnati alla tua squadra""".format(
                    emoji.emojize(":warning:", use_aliases=True)
                ),
            )
        # await bot.delete_message(message.chat.id,message.message_id)
...
async def process_motivo(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        query_motivo = "UPDATE segnalazioni.t_incarichi_interni SET note_rifiuto='{}' WHERE id={};".format(
            message.text, incarico_assegnato2[0][3]
        )
        update_motivo = esegui_query(con, query_motivo, "u")
        query_stato = "INSERT INTO segnalazioni.stato_incarichi_interni(id_incarico, id_stato_incarico) VALUES ({}, 4)".format(
            incarico_assegnato2[0][3]
        )
        stato = esegui_query(con, query_stato, "i")
        # print('id={}'.format(incarico_assegnato2[0][1]))
        query_squadra = "UPDATE users.t_squadre SET id_stato=2 WHERE id={}".format(
            incarico_assegnato2[0][1]
        )
        ...
        query_storico = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento) 
            VALUES ({0}, ' Incarico {1} rifiutato dalla seguente squadra: {2} - <a class="btn btn-info" href="dettagli_incarico_interno.php?id={1}"> Visualizza dettagli </a>');""".format(
            incarico_assegnato2[0][4],
            incarico_assegnato2[0][3],
            incarico_assegnato2[0][2],
        )
        storico = esegui_query(con, query_storico, "i")
        query_log = "INSERT INTO varie.t_log (schema, operatore, operazione) VALUES ('segnalazioni','{}', 'Incarico interno {} rifiutato');".format(
            incarico_assegnato2[0][0], incarico_assegnato2[0][3]
        )
        log = esegui_query(con, query_log, "i")
        if incarico_assegnato2[0][5] == 3:
            ...
            query_notifica = "select telegram_id from users.utenti_sistema where id_profilo <= {} and telegram_id !='' and telegram_attivo='t';".format(
                incarico_assegnato2[0][5]
            )
        notifica = esegui_query(con, query_notifica, "s")
        # print(notifica)
        for tid in notifica:
            ...
            try:
                await bot.send_message(
                    tid[0],
                    "{} L'incarico interno assegnato alla squadra {} sulla segnalazione {} è stato rifiutato con le seguenti note: {}".format(
                        emoji.emojize(":x:", use_aliases=True),
                        incarico_assegnato2[0][2],
                        incarico_assegnato2[0][6],
                        message.text,
                    ),
                )
            except:
                print(
                    "Problema invio messaggio all'utente con chat_id={}".format(tid[0])
                )
...
async def send_rifiuto(message: types.Message):
    ...
    if registered_user == 1:
        ...
        if incarico_assegnato == 1:
            ...
            await message.reply(
                "Ciao {} hai rifiutato l'incarico {}. Per favore fornisci la motivazione digitando un breve testo.".format(
                    message.from_user.first_name,
...
async def process_chiudo_note(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        query_incarico2 = """select us.matricola_cf, vc.id, vc.nome_squadra, viilu.id, vii.id_lavorazione, vii.id_profilo, vii.id_segnalazione
            ...
            group by us.matricola_cf, vc.id, vc.nome_squadra, viilu.id, vii.id_lavorazione, vii.id_profilo, vii.id_segnalazione""".format(
            message.chat.id
        )
        incarico_assegnato2 = esegui_query(con, query_incarico2, "s")
        print(message.chat.id, incarico_assegnato2)
        ...
        query_stato_c = "INSERT INTO segnalazioni.stato_incarichi_interni(id_incarico, id_stato_incarico) VALUES ({}, 3)".format(
            incarico_assegnato2[0][3]
        )
        stato = esegui_query(con, query_stato_c, "i")
        # print('id={}'.format(incarico_assegnato2[0][1]))
        query_squadra_c = "UPDATE users.t_squadre SET id_stato=2 WHERE id={}".format(
            incarico_assegnato2[0][1]
        )
        ...
        query_storico_c = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento) 
            VALUES ({0}, ' Incarico interno {1} chiuso dalla seguente squadra: {2} con il seguente messaggio: <br><i>{3}</i><br> - <a class="btn btn-info" href="dettagli_incarico_interno.php?id={1}"> Visualizza dettagli </a>');""".format(
            incarico_assegnato2[0][4],
            incarico_assegnato2[0][3],
            incarico_assegnato2[0][2],
            message.text,
        ...
        query_log_c = "INSERT INTO varie.t_log (schema, operatore, operazione) VALUES ('segnalazioni','{}', 'Incarico interno {} chiuso');".format(
            incarico_assegnato2[0][0], incarico_assegnato2[0][3]
        )
...
async def send_chiudo(message: types.Message):
    ...
    if registered_user == 1:
        ...
        query_incarico = """select * from users.v_componenti_squadre vcs 
        left join segnalazioni.v_incarichi_interni_last_update viilu on vcs.id::text = viilu.id_squadra::text 
        where vcs.matricola_cf = '{}' and vcs.data_end is null and viilu.id_stato_incarico =2""".format(
            registered_user[0][0]
        )
        incarico_assegnato = esegui_query(con, query_incarico, "s")
        if incarico_assegnato == 1:
            ...
            await message.reply(
                "Ciao {} stai chiudendo l'incarico. Per favore fornisci una nota di chiusura digitando un breve testo.".format(
                    message.from_user.first_name
                )
            )
        else:
...
# Check orario inizio incarico interno è numerico
@dp.message_handler(
    lambda message: not message.text.isdigit(), state=Form.orarioPresidio
)
async def process_orario_invalid(message: types.Message):
    return await message.reply(
        "I minuti inseriti non sono validi, devi inserire un numero"
...
async def process_orario_presidio(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        inizio_incarico = datetime.now() + timedelta(
            minutes=int(data["orarioPresidio"])
        )
        timepreview = inizio_incarico.replace(second=0, microsecond=0).time()
        await bot.send_message(
            message.chat.id,
            md.text(
                md.text(
                    "Hai indicato",
                    md.bold(data["orarioPresidio"]),
                    "minuti quindi l'ora di inizio è",
                    md.bold(timepreview),
                    "circa.",
                ),
                md.text(
                    "Una volta terminato il presidio ricordati di chiuderlo digitando /stop.\n"
                ),
            ),
        )
    print(timepreview)
    ...
    query_presidio2 = """select us.matricola_cf, vc.id, vc.nome_squadra, vslu.id, vs.id_lavorazione, vs.id_profilo, vs.id_segnalazione
            ...
            where us.telegram_id = '{}' and vc.data_end is null and vslu.id_stato_sopralluogo=1""".format(
        message.chat.id
    )
    presidio_assegnato2 = esegui_query(con, query_presidio2, "s")
    print(presidio_assegnato2)
    query_time = (
        "UPDATE segnalazioni.t_sopralluoghi SET time_preview='{}' WHERE id={};".format(
            timepreview, presidio_assegnato2[0][3]
        )
    )
    time_inizio = esegui_query(con, query_time, "u")
    query_stato_presidio = "INSERT INTO segnalazioni.stato_sopralluoghi(id_sopralluogo, id_stato_sopralluogo, parziale) VALUES ({}, 2 , 'false');".format(
        presidio_assegnato2[0][3]
    )
    ...
    if presidio_assegnato2[0][6] != "":
        query_storico_presidio = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento)
            VALUES ({0}, ' Sopralluogo {1} preso in carico dalla seguente squadra: {2} - <a class="btn btn-info" href="dettagli_sopralluogo.php?id={1}"> Visualizza dettagli </a>');""".format(
            presidio_assegnato2[0][4],
            presidio_assegnato2[0][3],
            presidio_assegnato2[0][2],
        )
        storico_presidio = esegui_query(con, query_storico_presidio, "i")
    query_log = "INSERT INTO varie.t_log (schema, operatore, operazione) VALUES ('segnalazioni','{}', 'Presidio (o sopralluogo) {} preso in carico');".format(
        presidio_assegnato2[0][0], presidio_assegnato2[0][3]
    )
    log = esegui_query(con, query_log, "i")
...
async def send_accetto(message: types.Message):
    ...
    if registered_user == 1:
        ...
        where vcs.matricola_cf = '{}' and vcs.data_end is null and vslu.id_stato_sopralluogo =1;""".format(
            registered_user[0][0]
        )
        ...
        if presidio_assegnato == 1:
            ...
            await message.reply(
                "Ciao {} hai accettato il presidio {}. Tra quanti minuti sarai sul posto?".format(
                    message.from_user.first_name,
                    emoji.emojize(":thumbs_up:", use_aliases=True),
                )
            )
...
async def process_chiudo_note(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        query_presidio2 = """select us.matricola_cf, vc.id, vc.nome_squadra, vslu.id, vs.id_lavorazione
            ...
            group by us.matricola_cf, vc.id, vc.nome_squadra, vslu.id, vs.id_lavorazione""".format(
            message.chat.id
        )
        presidio_assegnato2 = esegui_query(con, query_presidio2, "s")
        print(message.chat.id, presidio_assegnato2)
        query_note = "UPDATE segnalazioni.t_sopralluoghi SET note_ente='{}', time_stop=now() WHERE id={};".format(
            message.text, presidio_assegnato2[0][3]
        )
        update_motivo = esegui_query(con, query_note, "u")
        query_stato_cp = "INSERT INTO segnalazioni.stato_sopralluoghi(id_sopralluogo, id_stato_sopralluogo) VALUES ({}, 3)".format(
            presidio_assegnato2[0][3]
        )
        stato = esegui_query(con, query_stato_cp, "i")
        # print('id={}'.format(incarico_assegnato2[0][1]))
        query_squadra_cp = "UPDATE users.t_squadre SET id_stato=2 WHERE id={}".format(
            presidio_assegnato2[0][1]
        )
        ...
        query_storico_cp = """INSERT INTO segnalazioni.t_storico_segnalazioni_in_lavorazione(id_segnalazione_in_lavorazione, log_aggiornamento) 
            VALUES ({0}, ' Presidio {1} chiuso dalla seguente squadra: {2} con il seguente messaggio: <br><i>{3}</i><br> - <a class="btn btn-info" href="dettagli_sopralluogo.php?id={1}"> Visualizza dettagli </a>');""".format(
            presidio_assegnato2[0][4],
            presidio_assegnato2[0][3],
            presidio_assegnato2[0][2],
            message.text,
        ...
        query_log_cp = "INSERT INTO varie.t_log (schema, operatore, operazione) VALUES ('sopralluoghi','{}', 'Presidio (o sopralluogo) {} chiuso');".format(
            presidio_assegnato2[0][0], presidio_assegnato2[0][3]
        )
...
async def send_chiudo(message: types.Message):
    ...
    if registered_user == 1:
        ...
        query_presidio = """select * from users.v_componenti_squadre vcs 
        left join segnalazioni.v_sopralluoghi_last_update vslu on vcs.id::text = vslu.id_squadra::text 
        where vcs.matricola_cf = '{}' and vcs.data_end is null and vslu.id_stato_sopralluogo=2""".format(
            registered_user[0][0]
        )
        presidio_assegnato = esegui_query(con, query_presidio, "s")
        if presidio_assegnato == 1:
            ...
            await Form.stop.set()
            await message.reply(
                "Ciao {} stai chiudendo il presidio. Per favore fornisci una nota di chiusura digitando un breve testo.".format(
                    message.from_user.first_name
                )
            )
        else:
            await bot.send_message(
                message.chat.id,
                """{} Al momento non risultano presidi assegnati alla tua squadra""".format(
                    emoji.emojize(":warning:", use_aliases=True)
                ),
            )
    else:
...
async def bot_echo_all(message: types.Message):
    await message.reply(
        "Hai questo messaggio senza utilizzare nessun comando. Il contenuto di questo messaggio pertanto verrà ignorato dal sistema."
    )
